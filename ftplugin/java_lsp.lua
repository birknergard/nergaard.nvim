...
-- Auto-reattach buffer to existing LSP if already running
vim.api.nvim_create_autocmd({ 'BufReadPost', 'BufNewFile' }, {
  pattern = '*.java',
  callback = function()
    local clients = vim.lsp.get_clients()
    for _, client in ipairs(clients) do
      if client.name == 'jdtls' and root_dir:match(client.config.root_dir) then
...
-- Optional: detach LSP when buffer is unloaded
  ...
  callback = function()
    ...
    for _, client in ipairs(clients) do
      if client.name == 'jdtls' then
        -- Only detach if no other buffers using this client
